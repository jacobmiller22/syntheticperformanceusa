...
Welcome to the engine room of your website (`brandonsoil.jacobmiller22.com`).
...
1. **The Request**: A customer clicks a link or types `https://brandonsoil.jacobmiller22.com` into their [Web Browser](https://developer.mozilla.org/en-US/docs/Learn/Getting_started_with_the_web/How_the_Web_works). The browser sends an [HTTP Request](https://developer.mozilla.org/en-US/docs/Web/HTTP/Overview) across the internet.
...
* **Windows**:
  ...
  3. When the installer asks: *"Choosing the default editor used by Git"*, select **Use Visual Studio Code as Git's default editor**.
...
git --version
```
*(Should output `git version 2.4x.x` or similar).*
...
3. Run the installer:
   ...
   > During installation, the installer may show a screen titled *"Tools for Native Modules"* with a checkbox: *"Automatically install the necessary tools (Chocolatey, Python, Visual Studio C++ Build Tools)"*.  
...
*Verify*: In your new terminal window, run:
```bash
node -v
npm -v
```
*(Node will print `v20.x` or `v22.x`, and `npm` will print `v10.x` or higher).*
...
3. **Install the Astro Extension**:
   ...
   - *(Recommended)* Search for `Tailwind CSS IntelliSense` and install it as well.
...
cd ~/projects
```
*(On Windows PowerShell, you can also run: `mkdir C:\Projects; cd C:\Projects`).*
...
* **Option A: If Hosted on GitHub**:
  ...
  cd brandonamsoil
  ```
  *(If the repository is private, you will be prompted to log in to your GitHub account or paste a GitHub Personal Access Token).*
...
npm install
```
*(Or, if you prefer `pnpm`: run `npm install -g pnpm` followed by `pnpm install`).*
...
2. Run:
   ...
   git pull
   ```
*(Git will update your local files so you are working on the latest version).*
...
1. **Highlight and copy** a distinct 3–5 word phrase on the page you want to change (e.g., *"Providing personal lubrication expertise"*).
...
> When you see `{dealerConfig.dealerName}` or `{dealerConfig.phone}`, that is a live electrical connection pulling data directly from `dealer.ts`. Leave the curly braces intact unless you want to replace the variable with static text.
...
2. **File Naming Rules**:
   ...
   * ❌ `Brandon's Shop Bay 2026!.JPG` *(Spaces and special characters cause broken links on web servers).*
...
git add .
```
*(The dot `.` tells Git to stage all modified files, including any newly added photos or pages, so nothing is left on the garage floor).*
...
3. **Live Verification**: Within 60 to 90 seconds, visit the live production URL on your phone or computer:
   **`https://brandonsoil.jacobmiller22.com`**  
...
* **Fix**: Run:
  ...
  git restore .
  ```
  *(This wipes out all unsaved local changes across all files and restores the last clean commit. Like rolling the vehicle back to its pre-service baseline).*
